...
def train(restore=False, testing=False):
    ...
    # dataset = tf.data.Dataset.from_tensor_slices((X,y,edge_map)).batch(batch_size)
    # iter = dataset.make_initializable_iterator()
    # features, labels, edge = iter.get_next()
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            # X_batch_op, y_batch_op, edge_op = data_generator(batch_size, train_list[0: 0 + batch_size], map_dict,
            #                                                  num_classes)
            # sess.run(iter.initializer, feed_dict={X: X_batch_op, y: y_batch_op, edge_map: edge_op})
            for i in range(step_count_train): # in one iteration
                X_batch_op, y_batch_op, edge_op = data_generator(batch_size, train_list[idx: idx + batch_size],
                ...
                if (i + 1) % valid_step == 0:
                        ...
                        for j in range(valid_iterations):
                            ...
                            iou_sum += step_iou
                            summary_loss.append(step_loss_valid)
                        ...
                        mean_summary_loss = sum(summary_loss) / valid_iterations
                        # save model based on evaluation results : validation loss
                        if mean_summary_loss < loss_history[-1]:
                            ...
                            loss_history.append(mean_summary_loss)
                            ops.append(0)
                        else:
                            ops.append(1)
