...
def group_norm(x, training, G=32, eps=1e-5, scope='group_norm') :
    """https://github.com/taki0112/Group_Normalization-Tensorflow"""
    with tf.variable_scope(scope, reuse=False):
        H, W, C = x.get_shape().as_list()[1:]
        # tf.reshape cant work with: N = None, tf.shape(x) gives a scalar tensor with the variable batch size
        N = tf.shape(x)[0]
        G = min(G, C)
...
def dense_block(input_dense, training):
    ...
    # conv1 = group_norm(net, groups=32, channels_axis=-1)
    conv1 = tf.layers.batch_normalization(net, training=training)
    conv1 = tf.nn.relu(conv1)  # 256,256,64
    feature_map1_conv1 = tf.layers.conv2d(conv1, 64, (5, 5), activation=None, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=scale))  # 256,256,64
    ...
    feature_map2_conv2 = tf.layers.conv2d(conv2, 64, (5, 5), activation=None, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=scale))  # 256,256,64
    ...
    output_feature_map = tf.layers.conv2d(conv_out, 64, (1, 1), activation=None, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=scale))  # 256,256,64
...
def quick_nat(X, training, num_classes):
    """
       QuickNAT Network 
       Args:
           X:  Input Image
    ...
    ## Bottleneck Layer
    bottleneck_layer = tf.layers.conv2d(pool4, 64, (5, 5), activation=None, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=scale))
    ...
    ## Classifier Block
    classifier = tf.layers.conv2d(dense_block8, num_classes, (1, 1), activation=None, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=scale))
