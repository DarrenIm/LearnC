...
def predict():
    ...
    with open('/home/xiaodong/dice.csv', 'w') as out:
        ...
        with tf.Session() as sess:
            ...
            saver = tf.train.import_meta_graph(path + '/model.ckpt-63.meta')
            saver.restore(sess, path + "/model.ckpt-63")
            ...
            for data, label, new_affine, filename in generator:
                data, label = data.astype(np.uint8), label.astype(np.uint8)
                slice_256 = []
                label_256 = []
                data_trans = np.expand_dims(np.transpose(normalize_data(data), (2,0,1)), -1)
                dataset = tf.data.Dataset.from_tensor_slices(data_trans).batch(batch_size)

                iter = dataset.make_one_shot_iterator()
                el = iter.get_next()

                for i in range(int(data.shape[0]/batch_size)):
                    X_test_op = sess.run(el)
                ...
                np_case = np.argmax(np.concatenate(slice_256, axis=0), -1).astype(np.uint8) # 256, 256, 256, 40 -> 256, 256, 256
                np_case = np.transpose(np_case, (1, 2, 0))
                print(np_case.shape, np_case.dtype)
                new_image = nib.Nifti1Image(np_case, affine=new_affine)
                nib.save(new_image, 'ds_test_prediction.nii')
                _image = nib.Nifti1Image(data, affine=new_affine)
                nib.save(_image, 'ds_test_input.nii')
                _label = nib.Nifti1Image(label, affine=new_affine)
                nib.save(_label, 'ds_test_label.nii')
